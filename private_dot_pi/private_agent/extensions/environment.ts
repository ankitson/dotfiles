...
export default function (pi: ExtensionAPI) {
  ...
  const ENV_INFO = [
    ...
    "- Run web servers on `0.0.0.0:<port>` where `<port>` is in the 3000-3100 range (these ports are published from the container out to the host). Always bind `0.0.0.0`, not `127.0.0.1`.",
    "- The public URL for these servers is http://dev.ankitson.com — a server on port `<port>` is reachable at `http://dev.ankitson.com:<port>`.",
    "- IMPORTANT: whenever you start a server or want the user to view a page, give them the `http://dev.ankitson.com:<port>` link. Do NOT hand the user `localhost`/`127.0.0.1`/`0.0.0.0` URLs — they are not on this machine and cannot reach those.",
